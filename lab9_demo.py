...
# ONLY FOR INSTRUCTORS - can set steering control mode (e.g., open-loop, BT controller, balance, etc.) with this flag
flag_steer = 0      # 0 = open-loop, 1 = BT controller, can add more for PID balance/steering as necessary
# not required for students, they can implement this how they please...
...
def calc_torque_conv(Tx,Ty,Tz):
    # TODO [LAB-07]: Calculate desired motor torques T1,T2,T3 from given Tx,Ty,Tz
...
def main():
    ...
    try:
        ...
        data = ["i t_now Tx Ty Tz u1 u2 u3 theta_x theta_y theta_z psi_1 psi_2 psi_3 dpsi_1 dpsi_2 dpsi_3 phi_x phi_y phi_z dphi_x dphi_y dphi_z"]
        dl.appendData(data)
        ...
        while True:
            ...
            try:
                ...
                # Calculate ball angular velocities (dphi_x/y/z)
                if i == 1:
                    dphi_x = 0
                    dphi_y = 0
                    dphi_z = 0
                else:
                    dphi_x = (phi_x - prev_phi_x) / DT
                    dphi_y = (phi_y - prev_phi_y) / DT
                    dphi_z = (phi_z - prev_phi_z) / DT
                prev_phi_x = phi_x
                prev_phi_y = phi_y
                prev_phi_z = phi_z
                ...
                if (flag_steer == 0):   # open-loop steering
                    # Lab9: duty cycles each 3sec, Tz = [0.75, 1.5, 2.25, 3.0]
                    if (t_now >= 12):
                        # End program after 12 seconds - stop motors first
                        ...
                        Ty = 0
                        Tz = 0.75
                if (flag_steer == 1):   # bluetooth control
                    Tx = js_R_y
                    Ty = js_R_x
                    Tz = trigger_R2 - trigger_L2

                # Calculate motor effort/commands from desired Tx,Ty,Tz motion
                # TODO [LAB-07]: Call method to calculate motor commands (u123) from axis torques (Txyz)
